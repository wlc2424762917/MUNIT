...
class UNIT_Trainer(nn.Module):
    ...
    def sample(self, x_a, x_b):
        ...
        for i in range(x_a.size(0)):
            ...
            # x_a_recon.append((self.gen_a.decode(h_a)+1)/2)
            # x_b_recon.append((self.gen_b.decode(h_b)+1)/2)
            x_ba.append(self.gen_a.decode(h_b))
            x_ab.append(self.gen_b.decode(h_a))
            # x_ba.append((self.gen_a.decode(h_b)+1)/2)
            # x_ab.append((self.gen_b.decode(h_a)+1)/2)
        x_a_recon, x_b_recon = torch.cat(x_a_recon), torch.cat(x_b_recon)
        x_ba = torch.cat(x_ba)
        x_ab = torch.cat(x_ab)
        self.train()
